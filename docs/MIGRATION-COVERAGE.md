...
and the `app_config` toggles. Enforced by `TestConfigDropsAreSettableAndRemoved`
and `TestCompleteness_settableDropsAreDocumented`.
...
them via `terraform import` (`../terraform-coverage-test/migrate/import`) rather
than config rewrite.
