...
func TestAccKionProjectDataSource_basic(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping long-running test in short mode")
	}

	rName := acctest.RandomWithPrefix(acctest.ResourcePrefix)
	dataSourceName := "data.kion_project.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccProjectDataSourceConfigBasic(rName),
...
func testAccProjectDataSourceConfigBasic(rName string) string {
	return fmt.Sprintf(`
...
data "kion_project" "test" {
  id = kion_project.test.id
}
`, rName)
}
