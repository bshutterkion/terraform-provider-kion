...
// sweepAzureAccount reads GetAccountIndex (not paginated: one call yields the whole
...
func sweepAzureAccount(_ string) error {
	...
	if err != nil {
		return fmt.Errorf("listing kion_azure_account: %w", err)
	}
	if resp, ok := out.(*generated.AccountListResponse); ok {
		items := resp.Data
		for _, item := range items {
			if item.ID.Set && sweepAzureAccountMatch(item) {
	...
	for _, id := range ids {
		if _, err := conn.Client.DeleteAccount(ctx, generated.DeleteAccountParams{ID: id}); err != nil {
			return fmt.Errorf("deleting kion_azure_account (%d): %w", id, err)
		}
	}
	return nil
}

func sweepAzureAccountMatch(item generated.Account) bool {
