...
| account\_ids | Account IDs contains a list of accounts in this project that will be accessible via this cloud access role. Accounts that do not match the cloud provider ID (if given) will be filtered | `list(number)` | `null` | no |
...
| aws\_iam\_policies | IDs of the AWS IAM policies attached to this role. Will be filtered if AWS Cloud Provider ID is not given. | `list(number)` | `null` | no |
...
| azure\_role\_definitions | IDs of the Azure Role Definitions attached to this role. Will be filtered if Azure Cloud Provider ID is not given. | `list(number)` | `null` | no |
| cloud\_provider\_ids | Cloud provider IDs that specify which CSPs this role will be used for. If none provided, assume all cloud providers. 1 for AWS, 2 for Azure, 3 for GCP | `list(number)` | `null` | no |
...
| gcp\_iam\_roles | IDs of the Google Cloud IAM roles attached to this role. Will be filtered if GCP Cloud Provider ID is not given. | `list(number)` | `null` | no |
...
| user\_group\_ids | IDs of the user groups allowed to use this role to access the AWS console. | `list(number)` | `null` | no |
| user\_ids | IDs of the users allowed to use this role to access the AWS console. | `list(number)` | `null` | no |
